...
def get_landmark(img, face_detector, shape_predictor):
    ...
    #     #人脸结果错误，
    #     return False
    shape = shape_predictor(img, dlib.rectangle(0, 0, 128, 128))
...
def main(f_start=0, f_end=0):
    ...
    img_dir_path = "D:\\SFTP\\MK\\\MK_201101_face\\facescrub_ori\\"
    dir_path = {
        "128": "D:\\SFTP\\MK\\\MK_201101_face\\facescrub_128_top10\\",
        "mask": "D:\\SFTP\\MK\\\MK_201101_face\\facescrub_128_top10_mask\\",
        "sun_visor": "D:\\SFTP\\MK\\\MK_201101_face\\facescrub_128_top10_sun_visor\\",
        "sun_glass": "D:\\SFTP\\MK\\\MK_201101_face\\facescrub_128_top10_sun_glass\\",
        "mask_sv": "D:\\SFTP\\MK\\\MK_201101_face\\facescrub_128_top10_mask_sv\\",
        "mask_gt": "D:\\SFTP\\MK\\\MK_201101_face\\facescrub_128_top10_mask_gt\\",
        "sun_visor_gt": "D:\\SFTP\\MK\\\MK_201101_face\\facescrub_128_top10_sun_visor_gt\\",
        "sun_glass_gt": "D:\\SFTP\\MK\\\MK_201101_face\\facescrub_128_top10_sun_glass_gt\\",
        "mask_sv_gt": "D:\\SFTP\\MK\\\MK_201101_face\\facescrub_128_top10_mask_sv_gt\\",
    }
    for i in dir_path:
    ...
    for dir_name in file_list:
        ...
        for img_name in img_list:
            img_list_count = img_list_count + 1
            if img_list_count > 10:
                break
            img_ori = cv2.imread(os.path.join(cur_img_dir,img_name), -1)
            img = cv2.resize(img_ori, dsize=(128,128), interpolation=cv2.INTER_AREA)

            cv2.imwrite(os.path.join(dir_path['128'], dir_name, img_name), img)
            shape = get_landmark(img, face_detector, shape_predictor)
            if not shape:
                print(img_name + "失败")
            else:
                ...
                cv2.imwrite(os.path.join(dir_path['mask'], dir_name, img_name), img_out)
                cv2.imwrite(os.path.join(dir_path['mask_gt'], dir_name, img_name), img_out_gt)
                ...
                cv2.imwrite(os.path.join(dir_path['mask_sv'], dir_name, img_name), img_out)
                cv2.imwrite(os.path.join(dir_path['mask_sv_gt'], dir_name, img_name), img_out_gt)

                img_sun_glass_modified, x_index, y_index = modify_sun_glass(img_sun_glass, shape)
                img_out = add_overlap(img, img_sun_glass_modified, x_index, y_index)
                img_out_gt = add_overlap(empty_img, img_sun_glass_modified, x_index, y_index)
                cv2.imwrite(os.path.join(dir_path['sun_glass'], dir_name, img_name), img_out)
                cv2.imwrite(os.path.join(dir_path['sun_glass_gt'], dir_name, img_name), img_out_gt)
                ...
                cv2.imwrite(os.path.join(dir_path['sun_visor'], dir_name, img_name), img_out)
                cv2.imwrite(os.path.join(dir_path['sun_visor_gt'], dir_name, img_name), img_out_gt)
