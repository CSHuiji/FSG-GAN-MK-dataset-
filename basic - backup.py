...
def main(f_start=0, f_end=0):
    ...
    img_dir_path = "E:\\lmk0824\\data\\prepared_image\\img_align_celeba_crop\\"
    img_dir_path_out_mask = "E:\\lmk0824\\data\\prepared_image\\img_align_mask_gt\\"
    img_dir_path_out_sun_visor = "E:\\lmk0824\\data\\prepared_image\\img_align_sun_visor_gt\\"
    img_dir_path_out_sun_glass = "E:\\lmk0824\\data\\prepared_image\\img_align_sun_glass_gt\\"
    img_dir_path_out_mask_sv = "E:\\lmk0824\\data\\prepared_image\\img_align_mask_sv_gt\\"

    print_per = 200
    counter = 0

    if img_dir_path_out_mask == "":
        img_dir_path_out_mask = img_dir_path
    # if img_dir_path_out_sun_visor == "":
    #     img_dir_path_out_sun_visor = img_dir_path
    ...
    for f_name in file_list:
        if check_name(f_name):
            if f_end != 0 and f_start < f_end:
                f_id_int = int(f_name[0:6])
                if f_id_int < f_start or f_id_int > f_end:
                    continue
            img = cv2.imread(img_dir_path + f_name, -1)
            shape = get_landmark(img, face_detector, shape_predictor)
            if not shape:
                print(f_name + "失败")
                cv2.imwrite(img_dir_path_out_mask + "MASK_" + f_name, img)
                #cv2.imwrite(img_dir_path_out_sun_visor + "SV_" + f_name, img)
            else:
                img_mask_modified, x_index, y_index = modify_mask(img_mask, shape)
                #img_out = add_overlap(img, img_mask_modified, x_index, y_index)
                img_out = add_overlap(empty_img, img_mask_modified, x_index, y_index)
                cv2.imwrite(img_dir_path_out_mask + "MASK_" + f_name, img_out)
                ...
                #img_out = add_overlap(img_out, img_sum_visor, x_index_sv, y_index_sv)
                cv2.imwrite(img_dir_path_out_mask_sv + "MASK_SV_" + f_name, img_out)

                img_sun_glass_modified, x_index, y_index = modify_sun_glass(img_sun_glass, shape)
                img_out = add_overlap(empty_img, img_sun_glass_modified, x_index, y_index)
                #img_out = add_overlap(img, img_sun_glass_modified, x_index, y_index)
                cv2.imwrite(img_dir_path_out_sun_glass + "SG_" + f_name, img_out)

                x_index_sv = shape.part(29).x + img_o_x_index
                y_index_sv = shape.part(29).y + img_o_y_index
                img_out = add_overlap(empty_img, img_sum_visor, x_index_sv, y_index_sv)
                #img_out = add_overlap(img, img_sum_visor, x_index_sv, y_index_sv)
                cv2.imwrite(img_dir_path_out_sun_visor + "SV_" + f_name, img_out)
