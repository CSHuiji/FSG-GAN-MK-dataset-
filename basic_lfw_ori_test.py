...
def main(f_start=0, f_end=0):
    ...
    img_dir_path = "D:\\SFTP\\MK\\MK_201104\\lfw_funneled\\"
    dir_path_basic = "D:\\SFTP\\MK\\MK_201104\\"
    dir_path = {
        "test": dir_path_basic + "lfw_testset\\",
        "mask": dir_path_basic + "lfw_testset_mask\\",
        "sun_visor": dir_path_basic + "lfw_testset_sun_visor\\",
        "sun_glass": dir_path_basic + "lfw_testset_sun_glass\\",
        "mask_sv": dir_path_basic + "lfw_testset_mask_sv\\",
        "mask_gt": dir_path_basic + "lfw_testset_mask_gt\\",
        "sun_visor_gt": dir_path_basic + "lfw_testset_sun_visor_gt\\",
        "sun_glass_gt": dir_path_basic + "lfw_testset_sun_glass_gt\\",
        "mask_sv_gt": dir_path_basic + "lfw_testset_mask_sv_gt\\",
    }
    for i in dir_path:
    ...
    for iii in range(total_number):
        ...
        cur_img_dir = os.path.join(img_dir_path, file_list[1][iii])
        if not os.path.isdir(cur_img_dir):
            continue
        for i in dir_path:
            if not os.path.exists(os.path.join(dir_path[i], file_list[1][iii])):
                os.makedirs(os.path.join(dir_path[i], file_list[1][iii]))

        cur_img_path = os.path.join(img_dir_path, file_list[0][iii])
        img_ori = cv2.imread(cur_img_path, -1)
        img = cv2.resize(img_ori, dsize=(256, 256))
        img_name = file_list[0][iii]
